...
void FeatureTracker::trackFeatures(
  ...
  if (show_images) {
    ...
    // waitKey(0); // This is usually done in track_features.cpp or the caller
  }
  if (save_images) {
    Mat img_keypoints_1, img_keypoints_2;
    drawKeypoints(img_1, keypoints_1, img_keypoints_1);
    drawKeypoints(img_2, keypoints_2, img_keypoints_2);
    imwrite(getOutputPath("keypoints_1.png"), img_keypoints_1);
    imwrite(getOutputPath("keypoints_2.png"), img_keypoints_2);
  }
  // ~~~~ end solution
  //
  ...
  if (save_images) {
    Mat img_matches;
    cv::drawMatches(img_1, keypoints_1, img_2, keypoints_2, good_matches, img_matches);
    imwrite(getOutputPath("good_matches.png"), img_matches);
  }
  // ~~~~ end solution
  //
  ...
  // ~~~~ begin solution
  std::vector<KeyPoint> matched_kp1, matched_kp2;
  for (const auto& match : good_matches) {
    matched_kp1.push_back(keypoints_1[match.queryIdx]);
    matched_kp2.push_back(keypoints_2[match.trainIdx]);
  }
  match_kp_1_kp_2 = {matched_kp1, matched_kp2};

  std::vector<uchar> inlier_mask;
  if (!good_matches.empty()) {
    inlierMaskComputation(matched_kp1, matched_kp2, &inlier_mask);
  }
  // ~~~~ end solution
  //
  // ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~
  ...
  //
  // ~~~~ begin solution
  if (show_images || save_images) {
    Mat img_inliers_outliers;
    std::vector<char> char_inlier_mask(inlier_mask.begin(), inlier_mask.end());

    // Draw all matches in red first (as outliers)
    cv::drawMatches(img_1, keypoints_1, img_2, keypoints_2, good_matches, img_inliers_outliers,
                    Scalar(0, 0, 255), Scalar::all(-1), std::vector<char>(),
                    DrawMatchesFlags::NOT_DRAW_SINGLE_POINTS);

    // Draw inliers in green on top
    cv::drawMatches(img_1, keypoints_1, img_2, keypoints_2, good_matches, img_inliers_outliers,
                    Scalar(0, 255, 0), Scalar::all(-1), char_inlier_mask,
                    DrawMatchesFlags::NOT_DRAW_SINGLE_POINTS | DrawMatchesFlags::DRAW_OVER_OUTIMG);

    if (show_images) imshow("Inliers and Outliers", img_inliers_outliers);
    if (save_images) imwrite(getOutputPath("inliers_outliers.png"), img_inliers_outliers);
  }
  // ~~~~ end solution
  //
...
void FeatureTracker::printStats() const {
  ...
  if (!output_dir_.empty()) {
    ...
    if (log_file.is_open()) {
      ...
      log_file << "Avg. Number of good matches: " << avg_num_good_matches_ << "\n";
...
void FeatureTracker::setOutputDir(const std::string &output_dir) {
  output_dir_ = output_dir;
}

std::string FeatureTracker::getOutputPath(const std::string &filename) const {
  if (output_dir_.empty()) {
    return filename;
  }
