...
class PoseEstimator : public rclcpp::Node {
 public:
  ...
  PoseEstimator() : Node("pose_estimator") {
    ...
    std::string file_path = "/home/bach/projects/VNAV-labs/lab6_ws/src/lab6/pose_error_estimator_" + std::to_string(pose_estimator_) + "_ransac_" + std::to_string(use_ransac_) + ".txt";
    error_log_file_.open(file_path);
    if (!error_log_file_.is_open()) {
      RCLCPP_ERROR(get_logger(), "Failed to open error log file: %s", file_path.c_str());
    } else {
      error_log_file_ << "timestamp rot_error_deg rot_chordal_error trans_abs_error trans_norm_error\n";
    }
  ...
  void calibrateKeypoints(const std::vector<cv::Point2f>& pts1,
    ...
    cv::undistortPoints(pts1, points1_rect, camera_params_.K, camera_params_.D, cv::noArray(), cv::Mat::eye(3, 3, CV_64F));
    cv::undistortPoints(pts2, points2_rect, camera_params_.K, camera_params_.D, cv::noArray(), cv::Mat::eye(3, 3, CV_64F));
  ...
  */
  void evaluateRPE(double timestamp,
                   const tf2::Transform& gt_t_prev_frame,
    ...
    // ************************ begin solution ************************
    Eigen::Isometry3d gt_isom = tf2TransformToIsometry(gt_relative_pose);
    Eigen::Isometry3d est_isom = tf2TransformToIsometry(est_relative_pose);

    Eigen::Matrix3d R_gt = gt_isom.rotation();
    Eigen::Matrix3d R_est = est_isom.rotation();
    Eigen::Vector3d t_gt = gt_isom.translation();
    Eigen::Vector3d t_est = est_isom.translation();

    // Relative rotation error (chordal distance)
    double rot_chordal_error = (R_gt - R_est).norm();

    // Rotation angle error in degrees
    Eigen::AngleAxisd error_aa(R_gt.transpose() * R_est);
    double rot_error_deg = error_aa.angle() * 180.0 / M_PI;
    if (rot_error_deg > 180.0) {
      rot_error_deg = 360.0 - rot_error_deg;
    }

    // Relative translation error
    double trans_abs_error = (t_gt - t_est).norm();
    double trans_norm_error = 0.0;
    if (t_gt.norm() > 1e-6 && t_est.norm() > 1e-6) {
      trans_norm_error = (t_gt.normalized() - t_est.normalized()).norm();
    }

    // Print
    RCLCPP_INFO(get_logger(), "Estimator: %d | RPE - Rot: %.4f deg, Chordal: %.4f, Trans Abs: %.4f, Trans Norm: %.4f",
                pose_estimator_, rot_error_deg, rot_chordal_error, trans_abs_error, trans_norm_error);
    RCLCPP_INFO(get_logger(), "t_gt: [%.6f, %.6f, %.6f], t_est: [%.6f, %.6f, %.6f]",
                t_gt.x(), t_gt.y(), t_gt.z(), t_est.x(), t_est.y(), t_est.z());

    // Log to file
    if (error_log_file_.is_open()) {
      error_log_file_ << timestamp << " " << rot_error_deg << " " << rot_chordal_error << " "
                      << trans_abs_error << " " << trans_norm_error << std::endl;
    }
    // ************************ end solution *****************************
  }
  ...
  void cameraCallback(const sensor_msgs::msg::Image::ConstSharedPtr& rgb_msg,
    ...
    // Check if prev_pose_ is empty (i.e. first frame)
    if (prev_pose_.pose.orientation.w == 0.0 && prev_pose_.pose.orientation.x == 0.0 &&
        prev_pose_.pose.orientation.y == 0.0 && prev_pose_.pose.orientation.z == 0.0) {
      prev_bgr = bgr.clone();
      ...
      case 0: {
        ...
        if (adapter_mono.getNumberCorrespondences() >= min_nr_of_correspondences) {
          if (!use_ransac_) {
            ...
            if (!Es.empty()) {
              cv::Mat Rmat, tmat;
              extractPose(Es, pts1, pts2, camera_params_, Rmat, tmat);
              relative_pose_estimate = cv2Pose(Rmat, tmat);
            }
            // ************************* end solution  *************************
          } else {
            // (TODO) With RANSAC
            ...
            // ************************* begin solution  *************************
            std::shared_ptr<RansacProblem> problem = std::make_shared<RansacProblem>(adapter_mono, RansacProblem::NISTER);
      ...
      case 1: {
        ...
        if (adapter_mono.getNumberCorrespondences() >= min_nr_of_correspondences) {
          if (!use_ransac_) {
            ...
            // ************************ begin solution ************************
            opengv::essential_t E = opengv::relative_pose::eightpt(adapter_mono);
            opengv::essentials_t Es;
            Es.push_back(E);
            cv::Mat Rmat, tmat;
            extractPose(Es, pts1, pts2, camera_params_, Rmat, tmat);
            relative_pose_estimate = cv2Pose(Rmat, tmat);
            ...
            // *********************** begin solution ***********************
            std::shared_ptr<RansacProblem> problem = std::make_shared<RansacProblem>(adapter_mono, RansacProblem::EIGHTPT);
            opengv::sac::Ransac<RansacProblem> ransac;
            ransac.sac_model_ = problem;
            ransac.threshold_ = 2.0 * (1.0 - cos(atan(sqrt(2.0) * 0.5 / 800.0)));
            ransac.max_iterations_ = 50;
      ...
      case 2: {
        ...
        if (adapter_mono.getNumberCorrespondences() >= min_nr_of_correspondences) {
          ...
          if (!use_ransac_) {
            // Without RANSAC (OPTIONAL)
            // ************************* begin solution *************************
            Eigen::Vector3d t_est = opengv::relative_pose::twopt(adapter_mono, true);
            opengv::transformation_t T_est;
            T_est.block<3, 3>(0, 0) = rotation;
            T_est.col(3) = t_est;
            relative_pose_estimate = eigen2Pose(T_est);
            // ************************* end solution *************************
          } else {
            // (TODO) With RANSAC
            // ************************* begin solution *************************
            std::shared_ptr<RansacProblemGivenRot> problem = std::make_shared<RansacProblemGivenRot>(adapter_mono);
      ...
      case 3: {
        ...
        if (adapter_3d.getNumberCorrespondences() >= min_nr_of_correspondences) {
          if (!use_ransac_) {
            // Without RANSAC (Optional)
            // ************************ begin solution  ************************
            opengv::transformation_t T = opengv::point_cloud::threept_arun(adapter_3d);
            relative_pose_estimate = eigen2Pose(T);
            // ************************ end solution **********************
          } else {
            // (TODO) With RANSAC
            // *********************** begin solution ***********************
            std::shared_ptr<RansacProblem3D> problem = std::make_shared<RansacProblem3D>(adapter_3d);
    ...
    evaluateRPE(rgb_msg->header.stamp.sec + rgb_msg->header.stamp.nanosec * 1e-9, gt_t_prev_frame, gt_t_curr_frame, est_t_prev_frame, est_t_curr_frame);
