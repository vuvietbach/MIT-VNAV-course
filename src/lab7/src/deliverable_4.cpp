...
class SfmNode : public rclcpp::Node {
 public:
  ...
  SfmNode() : Node("sfm_node") {
    ...
    graph.add(PriorFactor<Pose3>(Symbol('x', 0), camera_poses[0], priorNoise));
    ...
    for (const auto& meas : measurements) {
      size_t camera_idx = meas.first;
      const Keypoint& kpt = meas.second;
      graph.add(boost::make_shared<GenericProjectionFactor<Pose3, Point3, Cal3_S2>>(
          kpt.kpt_coords_, measurementNoise, Symbol('x', camera_idx), Symbol('l', kpt.lmk_idx_), K));
    }
    ...
    // Insert code below:
    noiseModel::Diagonal::shared_ptr landmarkPriorNoise =
        noiseModel::Diagonal::Sigmas(Vector3(0.1, 0.1, 0.1));
    graph.add(PriorFactor<Point3>(Symbol('l', 0), landmarks[0], landmarkPriorNoise));
