 ...
 public:
  ...
  void run() {
    ...
    if (use_mocap_) {
      ...
      for (const auto& measurement : mocap_measurements) {
        graph.add(boost::make_shared<MoCapPosition3Factor>(measurement.first, measurement.second, mocapNoise));
      }
      // End 3b.
    }
    ...
    // Use Levenberg-Marquardt method to optimize the initial values
    ...
    // Create the optimizer LevenbergMarquardtOptimizer given the parameters
